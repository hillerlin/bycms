<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:61:"D:\wamp\www\bycms/application/install\view\install\step2.html";i:1501570659;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8" />
		<title>创建数据库-bycms</title>
	...
	<body>
		<div class="border">
			...
			<div class="rule_title">bycms V1.0<span>安装协议</span></div>
			<div class="rules">
    ...
    <form action="<?php echo url('install/step3'); ?>" method="post">
        <div class="item">
			...
			<div>数据库连接类型，服务器支持的情况下建议使用mysqli</div>
			    ...
			    <div>数据库名</div>
                <input name="db[]" value="bycms" type="text">
           
			    <div>数据库用户名</div>
                <input name="db[]" value="bycms" type="text">
          
       			<div>数据库密码</div>
                <input name="db[]" value="1KV96xG6qor2oGk" type="password">
			...
			<div>数据库端口，数据库服务连接端口，一般为3306</div>
			   ...
			   <div>数据表前缀，同一个数据库运行多个系统时请修改为不同的前缀</div>
                <input name="db[]" value="bycms_" type="text">
        ...
        <div class="item">
            <h2>管理员帐号信息</h2>
  			    ...
  			    <div>用户名</div>
                <input name="admin[]" value="bycms" type="text">
              
           
			    <div>密码</div>
                <input name="admin[]" value="bycms" type="password">
             
			    <div>确认密码</div>
                <input name="admin[]" value="bycms" type="password">
          
			    <div>邮箱，请填写正确的邮箱便于收取提醒邮件</div>
                <input name="admin[]" value="bycms@qq.com" type="text">
